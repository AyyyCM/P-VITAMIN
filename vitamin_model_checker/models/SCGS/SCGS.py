...
class SCGS():
    def __init__(self):
        ...
        self.actions = []
        # NEW: store (state, action, next_state) -> probability
        self.transition_probabilities = {}  # dict not array!
    ...
    def read_file(self, filename):
        ...
        for line in lines:
            ...
            if line == 'Transition':
                ...
                current_section = 'Transition_Probabilities'
            ...
            # If not header, then read contents based on what section we are in
                ...
                # Parse probabilities: "HH:0.8" or just "HH" (default 1.0)
                parsed_values = []
                for item in values:
                    if ':' in item:
                        action, prob = item.split(':')
                        parsed_values.append(action)
                        # Store probability: (from_state, action, to_state) indexed later
                    else:
                        parsed_values.append(item)

                rows_graph.append(parsed_values)
            ...
            elif current_section == 'Transition_Probabilities':
                # Parse probability values: Try:0.8,Skip:0.2
                prob_row = []
                for entry in line.strip().split():
                    if ':' in entry:
                        # Split by comma first if multiple action:prob pairs
                        for pair in entry.split(','):
                            if ':' in pair:
                                action, prob = pair.split(':')
                                prob_row.append((action, float(prob)))
                            else:
                                prob_row.append(pair)
                    else:
                        prob_row.append(entry)
                rows_probs.append(prob_row)
        ...
        # Store transition probabilities from rows_probs
        for state_idx, prob_row in enumerate(rows_probs):
            for next_state_idx, prob_entry in enumerate(prob_row):
                if isinstance(prob_entry, tuple):
                    action, prob = prob_entry
    ...
    def get_transition_probability(self, from_state, action, to_state):
        """Get probability of transition from from_state via action to to_state"""
                    ...
                    for action in actions:
                        for i, agent in enumerate(agents):
                            if action[agent - 1] != 'I':  # idle condition
                                actions_per_agent[f"agent{agent}"].append(
                                    action[agent - 1])

        # Remove duplicates from each agent's action list
        for agent_key in actions_per_agent:
            actions_per_agent[agent_key] = list(
                set(actions_per_agent[agent_key]))

        return actions_per_agent

    # return the number of actions extracted in get_actions()
    def get_number_of_actions(self, agents):

        n = self.get_actions(agents)
        return len(n)
        ...
        with open(input_filename, 'r') as input_file, open(output_filename, 'w') as output_file:
            ...
            for line in input_file:
                ...
                if line == 'Transition':
                    ...
                    output_file.write(
                        ' '.join([str(elem) for elem in modified_graph[matrix_row]]) + '\n')
                    matrix_row += 1
                elif current_section == 'Transition' and matrix_row == len(modified_graph):
                    current_section = None
    ...
    def update_cgs_file(self, input_file, modified_file, tree, tree_states, unwinded_CGS):
        ...
        def update_transitions(lines, new_transitions):
            ...
            updated_lines = lines[:transition_start] + \
                new_transitions + lines[transition_end:]
        ...
        def update_name_state(lines, states):
            ...
            updated_lines = lines[:name_state_start] + \
                [states_line] + lines[initial_state_index:]
            return updated_lines

        def update_labelling(lines, labelling):
            labelling_start = lines.index("Labelling\n") + 1
            num_agents_index = lines.index("Number_of_agents\n")
            updated_lines = lines[:labelling_start] + \
                labelling + lines[num_agents_index:]
            return updated_lines
    ...
    def create_label_matrix(self, graph):
        label_matrix = []
        for i, row in enumerate(graph):
            label_row = [self.get_label(i) if isinstance(
                elem, str) and elem != '*' else None for elem in row]
    ...
    def matrixParser(self, n):
        for row in self.graph:
            ...
            if any(count == 0 for count in char_I_count):
                raise ValueError(
                    "Idle error: There has to be at least one 'I' for each row")

    def get_outgoing_transitions(self, state_idx):
        """Get all transitions from a state with probabilities"""
        transitions = []
        graph = self.get_graph()

        for next_state_idx, action in enumerate(graph[state_idx]):
            if action != 0 and action != '*':
                prob = self.get_transition_probability(
                    state_idx, action, next_state_idx)
                transitions.append(
                    (next_state_idx, action, prob if prob > 0 else 1.0))
